    ...
    struct rpc_arg const * arg)
{
    ...
    rpc_serialize_raw_u64(buffer, (uint64_t) stat_buffer->st_dev);
    ...
    rpc_serialize_raw_u64(buffer, (uint64_t) stat_buffer->st_blksize);
    ...
    struct rpc_arg const * arg)
{
    ...
    uint64_t dev = 0, ino = 0, nlink = 0, rdev = 0, size = 0, blksize = 0, blocks = 0;
    int result;

    {
        result = rpc_deserialize_raw_u64(buffer, &dev);
    }
    ...
    if (0 == result)
    {
        result = rpc_deserialize_raw_u64(buffer, &blksize);
    }
    ...
    if (0 == result)
    {
        ...
        stat_buffer->st_blksize = (blksize_t) blksize;
